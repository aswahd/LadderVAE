...
class OmniglotLoader(BaseDataLoader):
    def __init__(self, batch_size=64, train=False, transforms=None, shuffle=True, drop_last=False):
        ...
        if os.path.isdir(omniglot_path):
            ...
            raise FileNotFoundError(
                'Omniglot dataset not found. Please download it and put it under \'{}\''.format(omniglot_path))
        ...
        for p in [os.path.join(omniglot_path, 'images_background'), os.path.join(omniglot_path, 'images_evaluation')]:
            for task_path in sorted(os.listdir(p)):
                ...
                for i, cls_path in enumerate(sorted(os.listdir(task_path))):
                    ...
                    ims = [
                        cv2.resize(cv2.imread(os.path.join(cls_path, filename), cv2.IMREAD_GRAYSCALE), (28, 28)) / 255
                ...
                dataset = CustomDataset(data=torch.Tensor(task_images).float(), labels=torch.Tensor(task_labels).long(), transforms=transforms)
        ...
        dataset = CustomDataset(data=images, labels=new_labels, transforms=transforms)
